(function(){
  ...
  function ready(fn){
    if (document.getElementById('products-grid') && typeof window.allProducts !== 'undefined') return fn();
    var n = 0, t = setInterval(function(){
      n++;
      if (document.getElementById('products-grid') && typeof window.allProducts !== 'undefined') { clearInterval(t); fn(); }
      if (n > 100) clearInterval(t);
    }, 100);
  }
  ready(function(){
    if (!document.getElementById('phase-c')) {
    ...
    if (tabs && !document.getElementById('product-search')) {
      ...
      document.getElementById('product-search').addEventListener('input', function(){
        filterCategory(window._activeCategory || 'All');
      });
    }
    ...
    function upgradeWaLinks(){
      document.querySelectorAll('a.pcard-wa, a.spotlight-result').forEach(function(a){
        try {
          ...
          if (!title && a.querySelector) title = (a.querySelector('p')||{}).textContent || '';
    ...
    filterCategory(window._activeCategory);
  });
})();
